# TaskManager documentation

All repository documentation is **English only**.

## Guides

| Document | Description |
|----------|-------------|
| [guides/demo-guide.md](./guides/demo-guide.md) | Step-by-step demo walkthrough |

## Architecture and decisions

| Document | Description |
|----------|-------------|
| [architecture/ARCHITECTURE.md](./architecture/ARCHITECTURE.md) | System layers, auth, data model |
| [decisions/](./decisions/) | Architecture Decision Records |

## Engineering

| Document | Description |
|----------|-------------|
| [engineering-practice.md](./engineering-practice.md) | Implementation order and review bar |
| [WORKFLOW.md](./WORKFLOW.md) | Branch naming and PR conventions |
| [assets/](./assets/) | Demo screenshot / GIF checklist |
